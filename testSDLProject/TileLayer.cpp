...
TileLayer::TileLayer(int tilesize, int rowcount, int colcount, TileMap tilemap, TilesetList tilesets) : m_Tilesets(tilesets) 
{
	...
	{
		TextureManager::GetInstance()->Load(m_Tilesets[i].Name, "C:/Users/Rayen/OneDrive/Bureau/Studium/Vertiefung in der Programmierung/resources/Maps/" + m_Tilesets[i].Source); 
	}
	...
	for (unsigned int i = 0; i < m_RowCount; i++)
	{
		for (unsigned int j = 0; j < m_ColCount; j++)
		{
			...
			if (tileID == 0)
				...
				//if this tile is on the iss column 
